...
const Helpers2 = {
  ...
  setAutoUpdate(enable = requiredParam('enable')) {
    if (enable && !this._autoUpdate) {
      this.on('resize.Helpers2:autoUpdate', () => this.update())
      this._autoUpdate = true
    } else if (!enable && this._autoUpdate) {
      this.off('resize.Helpers2:autoUpdate')
  ...
  init() {
    ...
    // Bind init event
    this.off('init._Helpers2')
    this.on('init._Helpers2', () => {
      this.off('resize._Helpers2:redrawMenu')
      this.on('resize._Helpers2:redrawMenu', () => {
      ...
      if (typeof document.documentMode === 'number' && document.documentMode < 11) {
        this.off('resize._Helpers2:ie10RepaintBody')
        this.on('resize._Helpers2:ie10RepaintBody', () => {
...
if (typeof window !== 'undefined') {
  ...
  if (Helpers2.isMobileDevice() && window.chrome) {
    document.documentElement.classList.add('layout-menu-100vh')
  }

  // Update layout after page load
  if (document.readyState === 'complete') Helpers2.update()
  else
